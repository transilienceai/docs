...
- **[One-off run](concepts/scheduling.md#one-off-runs)** — A single scheduled run of an app at a future date and time.
...
- **[Run History](concepts/run-history.md)** — The page listing every session you have run, with status, duration, and a link into each one.
- **[Session](concepts/sessions.md)** — One run of an app. Has an ID, a status, a start time, and a folder of outputs.
- **[Session detail](concepts/run-history.md#session-detail)** — The per-session page that shows the report, files, timeline, and chat for that session.
- **[Session ID](concepts/sessions.md#session-ids)** — A timestamp-based identifier that uniquely names one session; appears in URLs like `/run-history/{sessionId}`.
